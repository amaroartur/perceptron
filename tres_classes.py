...
# Definindo as classes alvo que serão utilizadas
...
for train_size in train_sizes:
	for eta in learning_rates:
		for iterations in n_iterations:
			# Filtrando os dados para as classes alvo selecionadas
			X_filtered = X[np.isin(y, [label_encoder.transform([target_classes[0]])[0], label_encoder.transform([target_classes[1]])[0], label_encoder.transform([target_classes[2]])[0]])]
			y_filtered = y[np.isin(y, [label_encoder.transform([target_classes[0]])[0], label_encoder.transform([target_classes[1]])[0], label_encoder.transform([target_classes[2]])[0]])]
